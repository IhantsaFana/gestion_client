...
<div class="container container-fluid d-flex flex-column justify-content-center align-items-center pt-5">
    <div class="mb-5">
        <h1 class="text-info">Modifier la Facture #<?= $data['id'] ?></h1>
    </div>
    <form id="factureForm" action="<?php echo URLROOT; ?>/facture/edit/<?php echo $data['id']; ?>" method="post" style="width: 500px;">
        <div class="form-floating mb-3">
            <input type="text" name="client" class="form-control form-control-lg <?php echo (!empty($data['client_err'])) ? 'Non valide' : ''; ?>" value="<?php echo $data['client']; ?>" required>
            <span class="invalid-feedback"><?php echo $data['client_err']; ?></span>
            <label class="form-label" for="client">Nom du client</label>
        </div>
        <div class="form-floating mb-3">
            <input type="text" name="caissier" class="form-control form-control-lg <?php echo (!empty($data['caissier_err'])) ? 'Non valide' : ''; ?>" value="<?php echo $data['caissier']; ?>" required>
            ...
            <div class="col-md">
                <div class="form-floating">
                    <select class="form-select" id="etat" aria-label="etat" name="etat" required>
                        <option value="Facturée" <?php echo $data['etat'] == 'Facturée' ? 'selected' : ''; ?>>Facturée</option>
                        <option value="Payée" <?php echo $data['etat'] == 'Payée' ? 'selected' : ''; ?>>Payée</option>
                        <option value="Annulée" <?php echo $data['etat'] == 'Annulée' ? 'selected' : ''; ?>>Annulée</option>
                    </select>
                    <label class="form-label" for="etat">Etat de la commande</label>
                </div>
            </div>
        </div>
        <div class="text-end form-floating mb-3">
            ...
            <button class="btn btn-primary" type="submit"><i class="fas fa-check"></i> Confirmer</button>
        </div>
    </form>
</div>
...
function calculateReturn() {
    ...
    if (!isNaN(montant) && !isNaN(percu)) {
        ...
        if (percu < montant) {
            document.getElementById('percu').classList.add('is-invalid');
            document.getElementById('percu_err').innerText = "Le montant perçu doit être supérieur ou égal au montant.";
        } else {
            document.getElementById('percu').classList.remove('is-invalid');
            document.getElementById('percu_err').innerText = "";
            document.getElementById('retourne').value = retourne.toFixed(2);
        }
    }
}
